					...
					success : function (comment, response, status) {
						...
						if (comment) { 
							...
							$("#comment_commentId_edit").validatebox({
								required : true,
								missingMessage : "请输入影评id",
								editable: false
							});
							$("#comment_movieObj_movieId_edit").combobox({
								url:"Movie/listAll",
							    valueField:"movieId",
							    textField:"movieName",
							    panelHeight: "auto",
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#comment_movieObj_movieId_edit").combobox("select", comment.movieObjPri);
									//var data = $("#comment_movieObj_movieId_edit").combobox("getData"); 
						            //if (data.length > 0) {
						                //$("#comment_movieObj_movieId_edit").combobox("select", data[0].movieId);
						            //}
								}
							});
