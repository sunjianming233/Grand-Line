...
public class UserInfoController extends BaseController {
	...
	public void list(String user_name,String name,String birthday,String telephone,String userType,String regTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, name, birthday, telephone, userType, regTime, page);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, name, birthday, telephone, userType, regTime);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String user_name,String name,String birthday,String telephone,String userType,String regTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (userType == null) userType = "";
		if (regTime == null) regTime = "";
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, name, birthday, telephone, userType, regTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, name, birthday, telephone, userType, regTime);
	    /*获取到总的页码数目*/
	    int totalPage = userInfoService.getTotalPage();
	...
	public void OutToExcel(String user_name,String name,String birthday,String telephone,String userType,String regTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(user_name == null) user_name = "";
        if(name == null) name = "";
        if(birthday == null) birthday = "";
        if(telephone == null) telephone = "";
        if(userType == null) userType = "";
        if(regTime == null) regTime = "";
        List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name,name,birthday,telephone,userType,regTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "UserInfo信息记录"; 
        String[] headers = { "用户名","姓名","性别","用户照片","出生日期","用户电话","家庭地址","用户类型","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getName(),userInfo.getSex(),userInfo.getUserPhoto(),userInfo.getBirthday(),userInfo.getTelephone(),userInfo.getAddress(),userInfo.getUserType(),userInfo.getRegTime()});
        }
        /*
